...
CHECKPOINT="${CHECKPOINT:-$ARTIFACT_ROOT/training/runs/checkpoints/h3_speedfull_s3072/weights_final_step_12840.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-$ARTIFACT_ROOT/evaluation/history3/h3_speedfull_s3072}"
ORIGINAL_H5="${ORIGINAL_H5:-/opt/huawei/explorer-env/dataset/ag_data/data/world_model/quentinll/lewm-tworooms/tworoom.h5}"
NORMALIZER="${NORMALIZER:-}"
RUN_LABEL="${RUN_LABEL:-H3-SpeedFull E4}"
...
run_seed() {
  ...
  local replay_args=()
  local normalizer_args=()
  if [[ "$seed" != "42" ]]; then
    replay_args+=(--skip-catalog-replay)
  fi
  if [[ -n "$NORMALIZER" ]]; then
    normalizer_args+=(--normalizer "$NORMALIZER")
  fi
  echo "[$RUN_LABEL] seed=$seed physical_device=$device output=$output"
...
for ((offset=0; offset<${#SEEDS[@]}; offset+=${#DEVICES[@]})); do
  ...
  for ((index=0; index<${#pids[@]}; index++)); do
    if ! wait "${pids[$index]}"; then
      echo "[$RUN_LABEL] failed ${labels[$index]}" >&2
      exit 1
    fi
    echo "[$RUN_LABEL] completed ${labels[$index]}"
  done
done

echo "[$RUN_LABEL] raw seeds completed: ${SEEDS[*]}"
"$PYTHON_BIN" scripts/run_tworoom_history3_eval.py \
  ...
  --skip-catalog-replay \
  --reuse-existing
echo "[$RUN_LABEL] summary=$OUTPUT_DIR/e4_speed_ctx_n50x${#SEEDS[@]}.json"
