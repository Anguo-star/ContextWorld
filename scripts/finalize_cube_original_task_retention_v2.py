#!/usr/bin/env python3
"""Seal the recovered Cube v4r1 OSMesa retention decision."""
...
from contextworld.benchmarks.cube_original_task_retention_v2 import (  # noqa: E402
...
def finalize(
    *, prereg_path: Path, result_path: Path, output: Path
    ...
    prereg = load_cube_cem_retention_v2_prereg(
    ...
    if result_path.resolve() != expected_result:
        raise RuntimeError("Cube CEM v2 result path drifted")
    if output.resolve() != planned_output:
        raise RuntimeError("Cube CEM v2 decision path drifted")
    if output.exists():
        raise FileExistsError(f"Refusing to overwrite Cube CEM v2 decision: {output}")

    validated = validate_cube_cem_v2_retention_result(
        prereg,
    ...
    if (
        ...
        or matrix.get("preregistration_id") != CUBE_CEM_RETENTION_V2_ID
        ...
        raise RuntimeError("Cube CEM v2 matrix report is incomplete")

    passed = bool(result["passed"])
    decision = {
        "schema_version": 1,
        "decision_id": "cube_gripper_carry_h3_v4r1_cem_retention_v2",
        "preregistration_id": CUBE_CEM_RETENTION_V2_ID,
        ...
        "authorization_chain": {
            ...
            "v1_failure_receipt": _identity(
                resolve_declared_path(
                    prereg["predecessor"]["failure_receipt"]["path"],
                    repo_root=ROOT,
                )
            ),
        },
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prereg", type=Path, default=DEFAULT_CUBE_CEM_RETENTION_V2_PREREG
    )
