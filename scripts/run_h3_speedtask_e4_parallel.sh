...
run_seed() {
  ...
  echo "[H3-SpeedTask E4] seed=$seed physical_device=$device output=$output"
...
for ((offset=0; offset<${#SEEDS[@]}; offset+=${#DEVICES[@]})); do
  ...
  for ((index=0; index<${#pids[@]}; index++)); do
    if ! wait "${pids[$index]}"; then
      echo "[H3-SpeedTask E4] failed ${labels[$index]}" >&2
      exit 1
    fi
    echo "[H3-SpeedTask E4] completed ${labels[$index]}"
  done
done

echo "[H3-SpeedTask E4] raw seeds completed: ${SEEDS[*]}"
"$PYTHON_BIN" scripts/run_tworoom_history3_eval.py \
  ...
  --skip-catalog-replay \
  --reuse-existing
echo "[H3-SpeedTask E4] summary=$OUTPUT_DIR/e4_speed_ctx_n50x${#SEEDS[@]}.json"
