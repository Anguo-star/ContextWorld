#!/usr/bin/env python3
"""Freeze paired-geometry loss divisors on Contact Friction Training only."""
...
DEFAULT_OUTPUT = (
    ...
    "pusht_contact_friction_h3_strict_v3/"
    "contact_friction_center_barrier_training_scales.json"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--original-h5",
        type=Path,
        default=contact.DEFAULT_ORIGINAL_H5,
    )
...
def main() -> None:
    ...
    if manifest.get("passed") is not True or expected_pairs != 8192:
        raise RuntimeError("Expected the audited 8,192-pair v3 Training set")
    if manifest["splits"]["train"].get("passed") is not True:
        raise RuntimeError("The v3 Training split audit did not pass")
    ...
    training = contact._training_split(
