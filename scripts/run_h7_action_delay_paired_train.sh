...
case "$FAMILY" in
  lewm)
    MODEL_ID=H7_ActionDelay_Paired_LeWM
    CONFIG="$ROOT/configs/benchmark/tworoom_action_delay_h7_paired_lewm_v1.yaml"
    ;;
  pldm)
    MODEL_ID=H7_ActionDelay_Paired_PLDM
    CONFIG="$ROOT/configs/benchmark/tworoom_action_delay_h7_paired_pldm_v1.yaml"
    ;;
  *)
...
RUN_NAME="${RUN_NAME:-h7_action_delay_paired_${FAMILY}_formal_s${TRAINING_SEED}}"
...
echo "[action-delay-h7-paired] family=$FAMILY seed=$TRAINING_SEED"
...
echo "[action-delay-h7-paired] report=$REPORT"
