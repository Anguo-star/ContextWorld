...
BENCHMARK_CONFIG="$ROOT/configs/benchmark/tworoom_speed_task_v1.yaml"
MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/contextworld-matplotlib}"
export MPLCONFIGDIR

case "$MODE" in
  preflight)
    RUN_NAME="${RUN_NAME:-h3_speedtask_s${TRAINING_SEED}}"
    ...
    ;;
  smoke)
    RUN_NAME="${RUN_NAME:-h3_speedtask_smoke_$(date -u +%Y%m%dT%H%M%SZ)}"
  ...
  resume)
    ...
    ;;
  *)
    echo "Usage: bash scripts/run_h3_speedtask_train.sh [preflight|smoke|fresh|train|resume]" >&2
    exit 2
    ;;
esac
...
echo "[H3-SpeedTask] mode=$MODE run=$RUN_NAME train_seed=$TRAINING_SEED split_seed=$DATA_SPLIT_SEED"
...
echo "[H3-SpeedTask] report=$REPORT"
echo "[H3-SpeedTask] log=$LOG"
