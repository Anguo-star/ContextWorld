...
BENCHMARK_CONFIG="$ROOT/configs/benchmark/tworoom_speed_isolated_v2.yaml"
MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/contextworld-matplotlib}"
export MPLCONFIGDIR

case "$VARIANT" in
  single)
    MODEL_ID=M_speed_single_v2
    RUN_PREFIX=h3_speed_single_v2
    ;;
  multi)
    MODEL_ID=M_speed_multi_v2
    RUN_PREFIX=h3_speed_multi_v2
    ;;
  *)
    echo "MODEL_VARIANT must be single or multi" >&2
    exit 2
    ;;
esac
...
case "$MODE" in
  ...
  resume)
    ...
    ;;
  *)
    echo "Usage: bash scripts/run_h3_speed_isolated_train.sh [preflight|smoke|fresh|train|resume]" >&2
    exit 2
    ;;
esac
...
echo "[speed-isolated-v2] variant=$VARIANT mode=$MODE run=$RUN_NAME seed=$TRAINING_SEED"
...
echo "[speed-isolated-v2] report=$REPORT"
echo "[speed-isolated-v2] log=$LOG"
