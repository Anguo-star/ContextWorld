...
case "$FAMILY" in
  lewm)
    MODEL_ID=H7_ActionDelay_FullRange_LeWM
    CONFIG="$ROOT/configs/benchmark/tworoom_action_delay_h7_full_range_lewm_v2.yaml"
    ;;
  pldm)
    MODEL_ID=H7_ActionDelay_FullRange_PLDM
    CONFIG="$ROOT/configs/benchmark/tworoom_action_delay_h7_full_range_pldm_v2.yaml"
    ;;
  *)
...
RUN_NAME="${RUN_NAME:-h7_action_delay_full_range_${FAMILY}_formal_s${TRAINING_SEED}}"
...
echo "[action-delay-h7-full-range] family=$FAMILY seed=$TRAINING_SEED"
...
echo "[action-delay-h7-full-range] report=$REPORT"
