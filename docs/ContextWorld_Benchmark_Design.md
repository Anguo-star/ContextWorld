...
必须同时检查多个真实执行预算和多个模型规划视野；不会改变完成时间尺度的因子，
可以使用单一主预算。是否采用资源阶梯由因子的作用机制决定，不按任务名称简单
划分。
...
模型视野、候选数或迭代次数等规划配置之间不能分摊样本。每个配置、每个 Eval、
每个 condition 都需要完整 `50×6=300` 个观测。配置对照应保持 query、上下文、
初始 CEM seed 和未被研究的参数一致。
...
完整执行预算曲线至少在 `H_standard` 上运行；视野阶梯在固定的标准或宽松执行
预算下运行。最慢因子条件在 `H_long` 内应通过精确动力学可达性检查。对照中保持
receding horizon、cost、候选数和迭代次数不变，避免把多项资源同时改变。

如果差异随 `B_exec` 增加而消失，说明真实截止有贡献；如果只随 `H_model` 增加而
消失，说明模型内部可达性是主要原因。两者都不消失时，再检查搜索预算、代价函数、
滚动重规划和模型误差。任何有限资源配置都不能称为“无限规划”。

使用 terminal-only cost 时，延长 `H_model` 还可能把候选的目标对齐时间推迟到
新的 horizon 末端。因此，视野更长带来的成功率提高不自动等于到达更快，必须同时
报告 steps-to-success 和轨迹指标。

最终 `B_exec`、`H_model` 和搜索预算必须在未用于主结论的新 Calibration catalog
上联合校准，并在 Test 前冻结。不能根据已经看过的 Validation 分数选择最有利于
某个上下文条件的配置。
...
模型视野或搜索预算的单因素对照应按同一 query 计算
“新配置下的条件差异 − 基线配置下的条件差异”，并报告配对区间。若增加搜索量
普遍提高所有条件的成功率，却没有稳定缩小条件差异，只能说明搜索质量改善，不能
把原差异归因于搜索不足。
...
| 资源 | model horizon、receding horizon、search budget、execution budget、radius |
| 统计 | 效应量、区间或 p 值 |
...
1. 三条件多步预测误差；
2. 三条件固定候选排序与真实代价；
3. 固定执行预算下的 model-horizon 敏感性；
4. 紧/标准/宽松执行预算成功率曲线；
5. 最终距离、best distance 和 trajectory AUC；
6. 共同成功 query 的配对到达步数；
7. CEM 求解次数、候选评估量和实际计算量；
8. 原始能力保持结果。
