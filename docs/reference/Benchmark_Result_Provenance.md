...
逐训练种子 ICL（Development，`correct_future_rate` 为主分数）：
...
| 推手移动幅度 | PLDM | 50.00% | 50.20% | 50.00% | 50.07% ± 0.12pp | 50.59%–50.78% | n/a | 38.28%–47.66% |
| 机械臂质量 | LeWM | 84.77% | 85.74% | 85.55% | 85.35% ± 0.51pp | 87.70%–89.65% | 96.48%–98.83% | 79.30%–81.64% |
...
| 接触摩擦 | LeWM | 50.00% | 50.00% | 50.20% | 50.07% ± 0.12pp | 51.56%–53.71% | 50.00%–57.03% | 46.48%–49.61% |
| 接触摩擦 | PLDM | 50.20% | 50.39% | 49.80% | 50.13% ± 0.30pp | 49.22%–52.15% | 48.44%–51.56% | 45.31%–51.17% |
| 运动阻尼 | LeWM | 50.00% | 50.20% | 50.00% | 50.07% ± 0.12pp | 49.80%–53.71% | 48.83%–57.03% | 26.17%–30.47% |
...
| 传送门出口位置 | LeWM | 50.98% | 50.59% | 51.17% | 50.91% ± 0.30pp | 69.53%–74.61% | 100.00% | 33.98%–48.83%（bootstrap 下界 28.13%–35.55%） |
...
PLDM 基线 ±20.25pp）同一量级，不构成异常。相比之下，本节与主文档 5.1.1 节列出的
“旧结果 → 新结果”落差普遍在 30–70pp（如接触摩擦、运动阻尼、Cube、推手移动幅度、
动作延迟 LeWM），远超种子间方差——说明这些落差来自训练方法（`coja_v1` 对比
`native`）、训练协议（`v4r1` 对比 `joint_scratch_v1`）或批次间代码/数据快照差异，
...
尚未汇总进任何 `artifacts/evaluation/` 或 `configs/benchmark/*.json` 冻结工件；如需
将其提升为正式 scoreboard 行，需先补跑 Public Test 并通过 `contextworld-scoreboard`
相同的汇总流程生成对应工件。
