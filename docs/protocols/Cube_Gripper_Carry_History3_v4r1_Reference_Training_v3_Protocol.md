# Cube Gripper Carry History=3 v4r1 Reference-Training v3 Protocol

Status: `preregistered_before_reference_training`. This v3 protocol authorizes a
...
## v1/v2 infrastructure failures and v3 recovery

The frozen v1 matrix was invoked exactly once. All six jobs stopped in the
shared input resolver before data materialization, model construction, forward
or backward execution, or optimizer stepping. It produced zero checkpoints
and zero training reports. The archived failure receipt classifies this as an
input-resolver contract mismatch, not a data or model-training result: v1
declared only `local_source`, while the unchanged shared trainer consumes
`path` for H5/Lance inputs and `checkpoint` for initialization weights.

v1 is closed and non-retriable. v2 uses a new freeze receipt, training root,
Development-score root, and decision path. Its only functional change is to
add `path == local_source` for both upstream data inputs and
`checkpoint == local_source` for both initial checkpoints. The generated
data, upstream content identities, initial weights, model recipes, seed block,
4,096-step budget, score thresholds, and Public closure are unchanged. The v2
freezer verifies the complete v1 preregistration/freeze/matrix/failure chain
and the zero-step state before authorizing any v2 output.

The v2 resolver change worked: all six jobs materialized the frozen Training
and Loader Validation data, instantiated the correct 25-input model, and
strictly loaded the family initialization checkpoint. They then stopped before
optimizer construction or any forward pass because the pinned 875e
`ConditionalSIGReg` constructor predates the shared engine's
`include_unpaired` and `complete_haar_population` keyword arguments. The
shared engine eagerly creates that optional loss even though neither authorized
Cube recipe selects it. The archived v2 receipt records zero forward passes,
zero backward passes, zero optimizer steps, zero checkpoints, and zero training
reports.

v2 is also closed and non-retriable. v3 adds only a process-local,
false/false-only constructor adapter for the pinned `ConditionalSIGReg`; it
raises if either unavailable semantic path is requested. The shared trainer,
shared engine, pinned runtime, data, model recipes, seeds, optimizer budget,
gates, and Public policy remain unchanged. The v3 freezer now instantiates the
exact shared-engine constructor signature in addition to strict checkpoint
loading, so this compatibility gap must fail before a training root can exist.
...
The freezer additionally preflights the exact `path` and `checkpoint` aliases and the false/false pinned-loss constructor consumed by the shared engine, then writes one x-exclusive receipt. The formal trainer refuses to run
...
- reusing or modifying the archived v1 or v2 output namespaces.
