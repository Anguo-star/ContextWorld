...
| 合成 matched future-25 | 5 | Synth5Matched-v2 轨迹中相隔 25 步 | 只改变轨迹来源 |
...
- `correct`：speed=5 正确 context；
- `wrong`：冻结 catalog 中的 speed=7 错误 context。
...
| 训练模型 | 原始 future-25 | 合成 matched future-25 | E4-speed5 无上下文 | E4-speed5 正确 | E4-speed5 错误 |
|---|---:|---:|---:|---:|---:|
...
主归因的三个 fixed-speed-trained 模型，在“合成 matched − 原始”上的成功率差
...
| 合成 matched future-25 | 41.43 px | 40.15 px | 64.83 px | 79/300（26.33%） |
...
原始 future-25 的平均距离 42.87 px、合成 matched 的 41.43 px 在数值上接近
...
条件化后，四个模型以及 none/correct/wrong 三种条件都完全相同：
...
三种 context 条件的成功集合也完全一致，所有模型的 success effect 都是
0 pp、discordant pairs 都是 0。正确 context 对平均最终距离有小幅连续变化
...
为 speed=5 后，原始 future-25 与合成 matched future-25 仍都在约 88%–94%，
