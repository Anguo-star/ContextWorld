...
def _adapter(args: argparse.Namespace):
    ...
    adapter_class = (
        ...
        else StableWorldModelPLDMMotionDampingAdapter
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="Audit and score PushT History=3 Motion Damping ICL",
    )
    parser.add_argument(
        "--release-config", type=Path, default=DEFAULT_MOTION_DAMPING_RELEASE_CONFIG
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "info":
        ...
        payload = audit_motion_damping_icl_release(
            release_config=args.release_config, repo_root=ROOT, full=args.full
        )
    elif args.command == "eval":
        payload = evaluate_motion_damping_icl_model(
