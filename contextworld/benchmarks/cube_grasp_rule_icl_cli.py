...
def _adapter(args: argparse.Namespace):
    release = load_cube_grasp_rule_icl_release(args.release_config)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="Audit and score whether History=3 reveals if the gripper can hold the cube",
    )
    parser.add_argument(
        "--release-config", type=Path, default=DEFAULT_CUBE_GRASP_RULE_RELEASE_CONFIG
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "info":
        ...
        payload = audit_cube_grasp_rule_icl_release(
            release_config=args.release_config, repo_root=ROOT, full=args.full
        )
    elif args.command == "eval":
        payload = evaluate_cube_grasp_rule_icl_model(
        ...
        payload = score_cube_grasp_rule_icl_results(
